...
import type { Analysis, ReportType, Totals, UnitEconomics } from './types/analytics'

type Page = 'home' | 'subscription'

const emptyTotals: Totals = { revenue: 0, orders: 0, adSpend: 0, margin: 0, impressions: 0, clicks: 0, carts: 0, stock: 0, promoRevenue: 0, costTotal: 0, commissionTotal: 0, acquiringTotal: 0, logisticsTotal: 0, taxTotal: 0 }
...
function emptyAnalysis(): Analysis {
  return {
    ...
    rows: [],
    totals: emptyTotals,
    strategy: { source: 'rules', headline: '', focusProducts: [], risks: [], actions: [] },
    quality: { score: 0, recognizedReports: [], missingReports: [], warnings: [], suggestions: [] },
  }
}
...
export default function App() {
  ...
  useEffect(() => { refresh().catch(() => setServerMessage('Backend не запущен — загрузка отчётов временно недоступна')) }, [])
  ...
  const uploadPage = <>
    <section className="hero-panel"><div><p className="eyebrow">AI-платформа для продавцов маркетплейсов</p><h1>Стратег для маркетплейсов</h1><p className="hero-text">Загружаете один или несколько отчётов Ozon/WB — система объединяет продажи, рекламу, остатки и акции, считает маржу и готовит стратегию.</p><div className="hero-actions"><label className="upload-button">{loading ? 'Анализирую…' : 'Загрузить отчёты'}<input type="file" multiple accept=".csv,.xlsx,.xls,.ods,.png,.jpg,.jpeg,.webp" disabled={loading} onChange={(event) => onFileUpload(event.target.files)} /></label><button className="plain-button" onClick={() => setPage('subscription')}>Посмотреть подписку</button>{hasUploadedAnalysis && <span className="file-name">Источник: {analysis.fileName}</span>}</div><p className="server-message">{serverMessage}</p><div className="upload-hint"><strong>Можно загрузить пачкой:</strong><span>продажи + реклама + остатки + акции. Файлы объединятся в один отчёт по SKU.</span></div></div><aside className="strategy-card">{hasUploadedAnalysis ? <><span>Стратегия месяца · {strategy.source === 'ai' ? 'AI' : 'rules'}</span><strong>{strategy.headline}</strong><div className="export-actions"><a href={exportUrl(analysis.id, 'xlsx')}>XLSX</a><a href={exportUrl(analysis.id, 'pdf')}>PDF</a></div></> : <><span>Начните с загрузки отчётов</span><strong>После загрузки здесь появятся стратегия, выводы и экспорт.</strong></>}<div className="hero-price-list"><p>Подписка</p>{subscriptionPlans.map((plan) => <button key={plan.name} onClick={() => setPage('subscription')}><span>{plan.name} · {plan.term}</span><strong>{plan.price}</strong></button>)}</div></aside></section>
