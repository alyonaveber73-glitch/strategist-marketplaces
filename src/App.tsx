...
import type { Analysis, ProductMetric, Strategy, Totals, UnitEconomics } from './types/analytics'
...
function demoAnalysis(): Analysis {
  ...
  return { id: '', fileName: 'Демо-данные', createdAt: new Date().toISOString(), reportTypes: ['unknown'], rows: demoRows, totals: total, strategy }
}
...
export default function App() {
  ...
  async function onFileUpload(file: File | null) {
    if (!file) return
    setLoading(true)
    setServerMessage('Загружаю файл и нормализую отчёты…')
    try {
  ...
  return <main className="page-shell">
    <section className="hero-panel"><div><p className="eyebrow">AI-платформа для продавцов маркетплейсов</p><h1>Стратег для маркетплейсов</h1><p className="hero-text">Простой рабочий MVP: без регистрации, оплаты и базы данных. Загружаете отчёты Ozon/WB, система нормализует продажи, рекламу, остатки и акции, считает маржу и готовит стратегию.</p><div className="hero-actions"><label className="upload-button">{loading ? 'Анализирую…' : 'Загрузить отчёты'}<input type="file" accept=".csv,.xlsx,.xls,.ods" disabled={loading} onChange={(event) => onFileUpload(event.target.files?.[0] ?? null)} /></label><span className="file-name">Источник: {analysis.fileName}</span></div><p className="server-message">{serverMessage}</p></div><aside className="strategy-card"><span>Стратегия месяца · {strategy.source === 'ai' ? 'AI' : 'rules'}</span><strong>{strategy.headline}</strong>{analysis.id && <div className="export-actions"><a href={exportUrl(analysis.id, 'xlsx')}>XLSX</a><a href={exportUrl(analysis.id, 'pdf')}>PDF</a></div>}</aside></section>
    ...
    <section className="content-grid"><div className="panel wide"><div className="panel-header"><h2>ТОП товаров</h2><p>Продажи, ДДР, маржа, остатки и расходы по справочнику.</p></div><div className="table-wrap"><table><thead><tr><th>Товар</th><th>Категория</th><th>Продажи</th><th>ДДР</th><th>Маржа</th><th>Остаток</th></tr></thead><tbody>{[...rows].sort((a, b) => b.revenue - a.revenue).map((row) => <tr key={row.sku}><td><strong>{row.name}</strong><small>{row.sku}</small></td><td>{row.category}</td><td>{money(row.revenue)}</td><td>{percent(row.adSpend / Math.max(row.revenue, 1))}</td><td>{money(row.margin)}</td><td>{row.stock}</td></tr>)}</tbody></table></div></div><div className="panel"><h2>Справочник юнит-экономики</h2><div className="unit-form"><input placeholder="SKU" value={draftUnit.sku} onChange={(e) => setDraftUnit({ ...draftUnit, sku: e.target.value })} /><input placeholder="Название" value={draftUnit.name} onChange={(e) => setDraftUnit({ ...draftUnit, name: e.target.value })} /><input placeholder="Себестоимость" type="number" value={draftUnit.cost} onChange={(e) => setDraftUnit({ ...draftUnit, cost: Number(e.target.value) })} /><input placeholder="Комиссия" type="number" value={draftUnit.commission} onChange={(e) => setDraftUnit({ ...draftUnit, commission: Number(e.target.value) })} /><input placeholder="Эквайринг" type="number" value={draftUnit.acquiring} onChange={(e) => setDraftUnit({ ...draftUnit, acquiring: Number(e.target.value) })} /><button onClick={addUnit}>Сохранить SKU</button><label className="mini-upload">Импорт XLSX<input type="file" accept=".csv,.xlsx,.xls,.ods" onChange={(e) => importUnits(e.target.files?.[0] ?? null)} /></label></div><small>Записей: {units.length}</small></div><div className="panel"><h2>Риски</h2><ul className="insight-list">{strategy.risks.map((risk) => <li key={risk}>{risk}</li>)}</ul></div><div className="panel"><h2>Рекомендации на месяц</h2><ol className="action-list">{strategy.actions.map((action) => <li key={action}>{action}</li>)}</ol></div><div className="panel"><h2>История в памяти сервера</h2><ul className="history-list">{latestAnalyses.map((item) => <li key={item.id}><button onClick={() => setAnalysis(item)}>{item.fileName}<small>{item.reportTypes.join(', ')}</small></button></li>)}</ul></div></section>
  </main>
}
